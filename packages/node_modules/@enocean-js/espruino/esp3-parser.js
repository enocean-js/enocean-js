const u8CRC8Table = [
  
]
function toCRC8 (accumulator, currentItem) {
  return u8CRC8Table[(accumulator ^ currentItem)]
}
const WAIT_FOR_SYNC_BYTE = 0
const FILL_HEADER = 1
const CHECK_CRC8_HEADER = 2
const FILL_DATA_OPTIONALDATA = 3
const CHECK_CRC8_DATAS = 4

class Parser {
  constructor () {
    this.maxBufferSize = 1000
    this.state = WAIT_FOR_SYNC_BYTE
    this.currentESP3Packet = []
    this.emitter = {
      'data': function (x) {}
    }
  }
  getDataLength () {
    return this.currentESP3Packet[1] * 256 + this.currentESP3Packet[2]
  }
  write (buffer) {
    var chunk = buffer.split('').map(item => item.charCodeAt(0))

    for (var offset = 0; offset < chunk.length; offset++) {
      var byte = chunk[offset]
      switch (this.state) {
        case WAIT_FOR_SYNC_BYTE:
          if (byte === 0x55) {
            this.currentESP3Packet = [byte]
            this.state = FILL_HEADER
          }
          break
        case FILL_HEADER:
          this.currentESP3Packet.push(byte)
          if (this.currentESP3Packet.length < 5) {
            break
          }
          this.state = CHECK_CRC8_HEADER
          break
        case CHECK_CRC8_HEADER:
          if (this.currentESP3Packet.slice(1).reduce(toCRC8, 0) !== byte) {
            var syncCodeIndex = this.currentESP3Packet.findIndex(function (item, index) { return (item === 0x55) && (index > 0) })
            if (syncCodeIndex > 0) {
              this.state = FILL_HEADER
              while (syncCodeIndex > 0) {
                this.currentESP3Packet.shift()
                syncCodeIndex--
              }
              break
            }
            console.warn('WRONG_HEADER_CHECKSUM_ERROR')
            this.currentESP3Packet = []
            this.state = WAIT_FOR_SYNC_BYTE
            break
          }
          if (this.getDataLength() + this.currentESP3Packet[3] <= 0) {
            console.warn('ILLEGAL_PACKET_LENGTH_ERROR')
            this.currentESP3Packet = []
            this.state = WAIT_FOR_SYNC_BYTE
            break
          }
          this.currentESP3Packet.push(byte)
          this.state = FILL_DATA_OPTIONALDATA
          break
        case FILL_DATA_OPTIONALDATA:
          this.currentESP3Packet.push(byte)
          if (this.currentESP3Packet.length > this.maxBufferSize) {
            this.currentESP3Packet = []
            this.state = WAIT_FOR_SYNC_BYTE
            console.warn('BUFFER_OVERFLOW_ERROR')
            break
          }
          if (this.currentESP3Packet.length < this.getDataLength() + this.currentESP3Packet[3] + 6) {
            break
          }
          this.state = CHECK_CRC8_DATAS
          break
        case CHECK_CRC8_DATAS:
          this.state = WAIT_FOR_SYNC_BYTE
          if (this.currentESP3Packet.slice(6).reduce(toCRC8, 0) !== byte) {
            console.warn('WRONG_BODY_CHECKSUM')
            break
          }
          this.currentESP3Packet.push(byte)
          this.emitter.data(this.currentESP3Packet)
          break
      }
    }
  }
  on (evt, cb) {
    this.emitter[evt] = cb
  }
}

module.exports = {
  Parser: Parser
}
