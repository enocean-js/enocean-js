...
export default class ESP3Packet {
  ...
  set data (value) {
    ...
    var tmp = new ByteArray(7 + value.length + this.optionalLength)
    tmp[0] = 0x55
  ...
  set optionalData (value) {
    ...
    var tmp = new ByteArray(7 + this.dataLength + value.length)
    tmp[0] = 0x55
    tmp.set(this.header, 1)
    tmp.set(this.data, 6)
  ...
  fromJSON (packet) {
    this._raw = new ByteArray(7 + packet.header.dataLength + packet.header.optionalLength)
    this._raw[0] = 0x55
    var byte0 = packet.header.dataLength % 256
    var byte1 = (packet.header.dataLength - byte0) / 256
    this._raw.set([byte1, byte0], 1)
    this._raw[3] = packet.header.optionalLength
