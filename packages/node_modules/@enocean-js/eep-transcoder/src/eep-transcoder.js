...
export function eep2IP (eep) {
  ...
  var funcs = eepDesc.case[0].datafield.reduce((acc, item) => {
    if (!item.hasOwnProperty('reserved') && !(item.shortcut === 'LRNB')) {
      acc.push(item)
    }
    return acc
  }, []).map((item) => {
...
function getFieldType (datafield) {
  ...
  if (datafield.hasOwnProperty('range') && !datafield.hasOwnProperty('spread')) {
    return 'range'
  }
  if (datafield.hasOwnProperty('reserved')) {
    return 'reserved'
  }
  if (datafield.hasOwnProperty('spread')) {
    return 'spread'
  }
  return 'raw'
}
...
export function encodeData (data, options) {
  ...
  for (var name in data) {
    ...
    if (fieldType === 'range' || fieldType === 'spread') {
      var v = Math.round(setRangeValue(data[name], field, desc.datafield, payload))
      if (fieldType === 'spread') {
...
export function getTeachInInfo (telegram) {
  ...
  if (p.RORG === 0xd4) {
    ...
    var lsb = p.payload.getValue(29, 3).toString(2).padStart(3, '0')
